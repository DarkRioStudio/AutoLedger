...
- `v1.6.4` is now in release closeout. `GOAL-2200` has frozen the Free / Pro boundary through a platform-neutral Pro access policy contract; the Pro page, restore / manage subscription entry points, local email monthly free allowance, batch-candidate gate, advanced dedupe gate, C1 Cloudflare Worker, D1/R2/Queue resources, cloud-candidate API, and App-side PDFKit local conversion entry have landed. Production APNs secrets, App Store Server API secrets, ASC sandbox purchases, review material, and TestFlight end-to-end validation still require external-environment closeout.
