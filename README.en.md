...
- `v1.6.4` is now in release closeout. `GOAL-2200` has frozen the Free / Pro boundary through a platform-neutral Pro access policy contract; the Pro page, restore / manage subscription entry points, local email monthly free allowance, batch-candidate gate, advanced dedupe gate, C1 Cloudflare Worker, D1/R2/Queue resources, cloud-candidate API, and App-side PDFKit local conversion entry have landed. Cloudflare production secret names for App Store Server API and APNs have been verified; the 2026-06-29 manual smoke passed subscription activation, APNs push, Worker cloud inbox, cloud candidate to hotel stay, and final ledger posting. Subscription metadata, review material, lifecycle screenshots, and evidence archiving remain in closeout.
- `v1.7.0` is planned as ASC / App Store `1.6.0`: the home receipt-scan entry moves to live OCR with photo / library fallback, Korean UI localization and the `ko` recognition pack are added, a reusable `common-api` will support five-language (`zh-Hans` / `zh-Hant` / `en` / `ja` / `ko`) location-catalog updates, date-based exchange rates, and hotel-stay historical weather summaries, and Pro expands from hotel folio automation to broader ledger efficiency with advanced search, subscription anomaly alerts, monthly export packages, and advanced rule automation.
...
| v1.6.4 | 1.5.0 by default | In closeout | Personal Pro foundations: Free / Pro boundaries now land in `AutoLedgerProAccessPolicy`; `ProEntitlementManager`, Pro page, restore / manage subscriptions, local email monthly free allowance, batch-candidate gate, advanced dedupe gate, C1 Cloudflare Worker, D1/R2/Queue, cloud-candidate API, and App-side PDFKit conversion have landed; production secrets, ASC sandbox purchases, review material, and TestFlight end-to-end validation remain |
| v1.7.0 | 1.6.0 | Planned | Live OCR receipt scanning with photo / library fallback; Korean UI localization and `ko` recognition pack; `common-api` five-language (`zh-Hans` / `zh-Hant` / `en` / `ja` / `ko`) location-catalog updates, date-based exchange rates, and hotel historical weather summaries; Pro automation expansion with advanced search, subscription anomaly alerts, monthly export packages, and advanced rule automation while basic search, subscriptions, export, and history remain free |
