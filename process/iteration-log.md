...
更新日期：2026-09-23（ITER-470 全平台图标统一）
...
### ITER-470 全平台图标统一
- 日期：2026-09-23；IDEA-001；内部 v1.8.0 / 商店 1.7.0。
- 目标与完成：Watch、tvOS、visionOS 改用已批准 Locked A 矢量源，移除旧钱包闪电生成代码；圆形图标缩放留白，电视图标保留横向布局及三层结构，更新 Top Shelf。Mac Catalyst 补齐 16–1024 像素专用图标槽。
- 范围：平台 Assets、tools/app-icons、版本记录；iPhone/iPad 原三种外观 PNG 未改动，签名、版本、业务代码未改动。
- 验证：图标尺寸/引用/不透明底图/前景存在检查通过，iPhone 明暗像素与批准母图相同；iOS、macOS、watchOS、tvOS、visionOS 五类 actool 编译全部退出 0；已检查平台遮罩预览。没有重复业务回归或宣称真机图标缓存验收。
- 回滚：回退本次图标及生成工具；无数据迁移。CI、合并及候选构建以对应 PR / Apple 记录为准。

### ITER-469 商店 1.7.0 开发收口
- 日期：2026-09-22
- 所属版本：内部 v1.8.0 / ASC 1.7.0；IDEA-001
- 目标：按 Owner 要求完成剩余开发、优先替换卡顿版本，未修改部分暂不重复验证。
- 实现：待处理新增来源与可逆批量；月结检查、完成 / 重开及配置 / 备份；异步搜索和分析缓存；云清洗缓存 / 退避；服务端配额及匿名操作聚合。
- 兼容：新来源决策扩展字段，原字段保持旧类型；不新增 App 数据表或 CloudKit 字段。重开 tombstone 保留，避免旧远端重放。
- 验证：113 条定向离线断言、新增 Core XCTest、受影响 UI smoke、iOS workspace 构建；配额 SQLite 与 Dashboard 定向测试、两 Worker 类型检查。具体证据与外部边界见 `versions/v1.8.0-development-completion.md`。
- 未改动 / 未执行：签名、营销版本号、构建号、商店提交、公开发布、Studio / Gate / Worker 部署；D1 0004 只交付迁移文件。
- 回滚：源码回退；新增可选 JSON 字段允许旧客户端忽略，不删除生产数据。

### ITER-468 DeepSeek 旅程小记与天气事实
- 日期：2026-09-22
- 所属版本 / 来源：v1.8.0，IDEA-001；继续 `wit_01M33EVBB836108BR6YNR8HGXG`，Owner 在同一真机验证中追加酒店文案修复。
- 目标：消除用户截图中的 Summary 占位和“一晚写成那几晚”，按 Owner 要求由 DeepSeek 整理文案。
- 改动范围：Core 旅程请求 / 响应 codec、App 既有 DeepSeek 配置的专用调用、酒店详情、五语文案与回归。
- 完成内容：明确点击生成，最小入住事实输入，编辑后分享；无模板回退；未知天气与缺失降水如实呈现；变更输入清除旧草稿并取消旧请求。
- 测试情况：完整离线回归 1,184 项 PASS（含天气占位、退房边界、最小请求字段、原始天气精度归一和空 / 截断响应）；签名 workspace 构建、真机原位安装和启动通过。Owner 已确认真实 DeepSeek 生成成功，并指出初稿精度过细、语气像数据报告；本轮追加发送前精度归一与自然表述要求，最终文字仍待设备复核。
- 风险与注意事项：模型文案为可编辑草稿，不修改真实酒店记录或金额；不新增云端服务部署、Keychain 写入或秘密权限。仍须以真机实际生成回读确认外部 API，不能以请求 codec 回归冒充 API 成功。
- 回滚方式：回退本轮源码，无数据迁移。
- 结论：实现已完成，回归、SCM 与真机证据分开记录。

### ITER-467 收件箱真机热点与 Pro 标题
- 日期：2026-09-22
- 所属版本 / 来源：v1.8.0，IDEA-001；WorkIntent `wit_01M33EVBB836108BR6YNR8HGXG`。
- 目标：处理第一批优化后的真机残余热点，并修复用户截图中的数据清洗 Pro 门标题重复。
- 改动范围：InboxView、LedgerStore、DataCleaningSuggestionsView、离线回归及性能审计记录。
- 完成内容：全部语言快捷记账备注只构造一次；数量以 visibleTransactionsRevision 和候选集合为键复用，保持原有精确匹配语义；数据清洗改用既有 autoLedgerContentTitleNavigation。
- 测试情况：完整离线回归 1,171 项 PASS，含同 ID 备注编辑、添加、删除、账本切换及候选集合变化；500 / 5,000 / 20,000 条重复读取一致；physical iPhone workspace Debug 签名构建 PASS。
- 风险与注意事项：真机前测已经验证第一批在真实账本体验流畅，但仍有仪器标记停顿；本轮不把合成计算耗时当作点击延迟，也不改存储、权益或同步协议。
- 回滚方式：回退本轮源码，无数据迁移。
- 结论：本地构建与回归通过，真机复测及 SCM 状态见性能审计记录和交付回执。

### ITER-466 账本点击热路径缓存与 v1.8 剩余范围核查
- 日期：2026-09-22
- 所属版本 / 来源：v1.8.0 / IDEA-001 / GOAL-2430、2460、2490。
- 目标与范围：减少几百条账单时列表重算和 formatter 创建；核对新版本计划。修改 Core AppFormatters、LedgerStore、LedgerView、回归 / 基准脚本及版本文档。
- 完成：地区感知且加锁的有界 formatter 缓存；行文案复用；带数据 revision 与查询条件的列表快照；记录功能实现缺口与设备 / 发布证据缺口。
- 验证：完整离线回归 1,158 PASS、静态 smoke、generic iOS workspace 无签名 build；Swift 6 严格并发格式测试通过。500 条旧行格式化 227.18 ms，新行 1.62 ms；20,000 条首次关键词查询 157.600 ms，缓存命中平均 0.001713 ms（合成基准，非设备点击延迟）。
- 修正记录：新增回归初版误修改不可变 Transaction 字段，改为构造同 ID 新值；一次回归编译中测试文件改动导致结果无效，稳定源码后完整重跑通过。
- 未完成 / 风险：首次大规模搜索、真实设备点击 / Tab / 待处理中心性能、双设备同步与其它 v1.8 发布门禁保持开放，详见版本缺口核查。
- 回滚：回退本轮代码；没有数据 / schema / 权益变更，没有发布构建或部署。
- 治理：WorkIntent `wit_01M33A7S0QQ42107188ECGF730`，复用原 workspace，源码交付。
- 下一步：按同设备隔离数据采样确定后续异步计算与 UI 更新范围，再补 PendingAction 来源、批量处理和月结闭环。

### ITER-465 仓库分支 / stash 收敛与构建触发准备
- 日期：2026-08-28
...
- 日期：2026-08-04
- 所属版本：v1.8.0 / ASC 1.7.0
- 所属阶段：Internal TestFlight Feedback / Global Formatting
- 类型：Bugfix / 数据语义 / 测试
- 目标：修复记账与账本入口把交易金额继续按当前系统国家或地区币种渲染的问题，让默认消费币种只决定之后的新记录，并把每笔交易的币种固定在记账时点。
- 改动范围：交易币种格式化与旧记录兼容回填、默认消费币种和系统地区变化确认、iPhone / iPad / Mac 的记账、账本、首页、报表与同步冲突展示、五语文案、离线回归、全球格式 smoke、版本计划、CHANGELOG 与本日志。
- 未改动范围：不重算或批量转换任何既有金额，不改写原始金额 / 原始币种 / 汇率字段，不推进交易 `updated_at` 或同步 revision，不新增 SQLite / CloudKit / D1 schema，不修改 Worker、StoreKit、版本号、build number、签名、entitlement、ASC 在线状态、构建标签或产品标签；不纳入现有未跟踪素材目录。
- 完成内容：新手工交易按目标账本币种或默认消费币种写入 `ledgerCurrencyCode`；账单编辑优先保留交易已经固定的币种；账本行、回收站、首页、报表、iPad / Mac 工作区和同步冲突列表按交易自身币种显示；旧交易仅在币种字段为空时按其账本显式币种、否则按当时已接受的默认币种做一次本地元数据回填，金额和同步元数据保持不变；系统模式检测到地区币种变化时询问“以后新记录改用新币种”或“继续使用旧币种”，两种选择都明确不修改既有交易；同一视图存在多币种时，合计显示“多币种（未换算）”，不伪装为单一币种合计。
- 未完成内容：没有新增历史汇率换算、批量币种转换或多币种折算报表；旧版本从未保存币种的交易无法从现有数据反推出其真实历史币种，只能在升级时按账本显式币种或已接受默认币种固化；尚未生成新的 TestFlight 构建，也未完成 iPhone / iPad / Mac 同路径人工验收。
- 测试情况：`bash scripts/run_offline_regression.sh` 完整 PASS，覆盖默认币种、地区变化选择、旧交易回填、金额与同步元数据不变、编辑保持币种和混合币种合计；全球格式 smoke、本地化 release smoke、五语 `plutil -lint` 与 `git diff --check` PASS；Xcode 27 beta 使用 `.xcworkspace` 对 generic iOS 执行 `CODE_SIGNING_ALLOWED=NO` 构建并 `BUILD SUCCEEDED`。
- 风险与注意事项：升级前缺失交易级币种的历史记录没有可靠来源可恢复原币种；本轮采取的是可审计的一次性元数据固化，不声称恢复历史事实。多币种视图不做自动换算，若未来需要可比较合计，必须独立冻结汇率日期、来源、精度和回滚合同。
- 回滚方式：恢复系统地区动态 fallback 和原展示调用，移除地区变化确认、旧币种回填、五语文案及对应回归；已回填的 `ledger_currency_code` 是非破坏性交易元数据，回滚代码不应主动清空或改写金额。
- 结论：默认消费币种现已用于未来新记账，交易币种在写入后保持稳定；改变系统地区或默认币种不会批量改变、换算既有账单。
- 下一步建议：在下一版 TestFlight 先用同一笔 CNY 账单完成“改系统地区 → 保留旧默认 / 接受新默认 → 新增下一笔账 → 重启”人工验收，再分别检查 iPhone 记账 / 账本与 iPad / Mac 工作区；不要把本地构建通过当作设备和 TestFlight 已验证。
...
- 类型：能力增强 / 国际化 / 语音 / 日期安全 / Watch / 测试
- 目标：继续清理主 App 用户可见的人民币与中文语音默认，并让纯数字日期按地区顺序解析且在双解时进入人工复核。
- 改动范围：通知金额、Watch 快速记账与语音确认、支持页预览、酒店草稿 fallback、分享卡图标、默认货币目录、结构化 JSON 目标币种；Speech locale；`AppFormatters` 数字日期顺序与歧义检测；OCR 确认页五语警告；离线与静态回归。
- 未改动范围：保留人民币 OCR 识别规则、ASC 截图宿主中的固定演示金额、导出文件名、API ISO 日期、邮件协议日期和机器稳定时间戳；未修改 schema、Worker、StoreKit、ASC、版本号、build number、构建标签或产品发布标签。
- 完成内容：快速记账成功通知不再固定 `¥`，由识别币种或系统地区币种格式化；Watch 快速输入和语音确认使用 iPhone 同步的默认写入账本币种，并按币种小数位显示；月报分享卡与金额问题图标改为通用 banknote。
- 完成内容：语音识别由固定 `zh_CN` 改为跟随 App 语言：简中 `zh-CN`、繁中 `zh-TW`、日语 `ja-JP`、韩语 `ko-KR`，英语优先保留英语系统地区，否则回退 `en-US`；系统语言模式继续跟随系统 locale。
- 完成内容：`04/07/2026` 之类数字日期在 `en-US` 按月/日解析，在 `en-GB` 按日/月解析；当日期的前两个分量都不超过 12 且不同，OCR 确认页显示五语复核警告，仍由用户通过现有 DatePicker 最终确认，不静默猜测后直接保存。
- 完成内容：新增 `scripts/check_global_format_smoke.py` 并接入完整离线回归；门禁执行前按预期发现 13 项旧硬编码/缺失合同，修复后全部通过。普通运行时审计只剩语言选择中的明确 zh locale；人民币识别和截图演示样例保持白名单边界。
- 未完成内容：`GOAL-2460` 尚未关闭；美英加澳新真实设备语音可用性、英语票据 / 酒店样本、税费与服务费、12/24 小时、截图和 ASC 地区验收仍待后续证据。
- 测试情况：`bash scripts/run_offline_regression.sh` PASS，新增美英日期顺序与歧义检测断言且历史酒店日期回归保持通过；`check_global_format_smoke.py`、Widget smoke、本地化覆盖与 `git diff --check` PASS；iOS generic workspace build PASS，只保留工程既有 Swift 6 渐进迁移、MediaPipe 与 Watch API 弃用 warning。
- 风险与注意事项：无明确币种的旧通知或旧 Watch 快照仍只能回退设备系统币种；纯数字日期即使按 locale 解析也不能证明票据原始语义，因此双解日期必须保持人工提示；语言 override 变化后新建语音识别器才采用新 locale。
- 回滚方式：可独立回退日期解析顺序、歧义提示、Speech locale 与各展示点；未新增持久化字段，回滚不需要数据迁移或清库。
- 结论：主 App、通知、语音和 Watch 的第二批全球默认已完成，歧义日期从静默月优先变为地区解析加显式复核。
- 下一步建议：停止继续扩大 `GOAL-2460` 代码面，先建立美英加澳新设备 / 样本验收矩阵；工程主线转入 `GOAL-2410`，先冻结 PendingAction Core 合同和稳定 identity，不直接写 SQLite / CloudKit schema。
...
### ITER-470 Actions 邮件轮询与 macOS 成本优化
- 日期：2026-09-23
- 类型：CI 维护
- 目标：减少邮件空轮询和纯文档改动的 macOS 用量。
- 范围：4 个 Actions workflow；CHANGELOG 与本日志。
- 实现：15 分钟轮询改为每小时第 17 分钟；共享 Linux 分类 job 使用 PR merge-base / push before 差异，只有纯文档 Markdown 才跳过原生检查；手动触发保留完整检查。静态检查移到 Linux，现有 macOS 检查名称不变。
- 本地验证：actionlint v1.7.7、git diff --check、本地化覆盖和布局静态检查通过；8 个分流场景通过，包含文档、混合改动、源代码、workflow、脚本、非 Markdown 文档资源、空差异和手动触发。
- 交付边界：本条记录本地验证；远端 CI 与合并状态以 PR 为准。
- 风险：邮件反馈通常最多等待一个小时轮询周期，GitHub 调度仍可能延迟；代码变更仍执行完整原生检查。
- 回滚：revert 本轮 CI 维护提交。
