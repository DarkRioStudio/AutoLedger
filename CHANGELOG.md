...
### 修复（v1.5.0）
- [2026-06-03 +0800] 修正 Apple Watch accessory corner 表盘小组件样式：改用 `.widgetLabel { Gauge(...) }` 让 watch face 沿表盘边缘渲染支出程度条，角落主体只保留今日金额读数；隐藏 current / min / max 可见标签，避免自绘横条被压在角落内容区域内；不修改 Widget family、target、Bundle ID、App Group 或同步数据源。
