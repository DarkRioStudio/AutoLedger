...
- [2026-04-13 +0800] 修复淘宝闪购订单进行中截图商户误识别 bug：`ReceiptParser` 新增 `parseTaobaoFlashOrder(lines:)` 专用方法，检测"骑士"+"闪购"组合（淘宝即时配送订单进行中页特征），在"闪购"标签行之后搜索含中文括号"（）"的店铺名称行，去除尾部导航箭头（＞）后返回正确商户名（如"和府捞面（杭州空港新天地卫星店）"）；`parse()` 商户优先链新增 `taobaoFlashMerchant`；fallback 商户提取 skip 列表新增"外卖红包"、"骑士"、"催单"，防止外卖 UI 噪声行被误识别为商户名；新增"淘宝闪购订单进行中截图"回归样本及预期值（merchant=和府捞面（杭州空港新天地卫星店）, amount=47.4, category=dining）。
...
- [2026-04-11 +0800] 修复支付宝支付成功页"回首页"按钮被误识别为商户名：`ReceiptParser` fallback 商户提取 skip 列表新增"回首页"（支付宝页面导航按钮）；fix 后"麦当劳湖州德清米兰洲际酒店餐厅"等真实商户可正常提取，分类自动命中 .dining；新增"支付宝麦当劳支付成功截图"回归样本及预期值。
...
### 变更
- [2026-04-10 +0800] 移除冷启动预置样例数据（盒马鲜生/滴滴出行/Apple Services），新安装后账本为空。
