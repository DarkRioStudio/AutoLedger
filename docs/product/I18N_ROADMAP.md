...
> 文档状态：Canonical
> 真源范围：跨版本语言分组、主语言策略、发布准入门禁和地区识别扩展顺序
> 最后核验：2026-07-17
...
AutoLedger 从 `v1.7.0 / ASC 1.6.0` 起采用固定双主线发布：

> **每个公开功能版本同时交付一个产品主题和一组新语言 / 地区能力。**

语言组不是 `Localizable.strings` 的数量承诺。一个语言只有在商店、界面、识别、样本、地区票据和人工审校六项均达到发布门禁后，才可以在 App Store 和公开文案中标记为支持。

该原则不改变当前 `v1.7.0` 的范围：本版本新增语言仍只有韩语；西班牙语和巴西葡语从 `v1.8.0` 开始实施。
...
| `v1.8.0` | ASC `1.7.0` | Review & Close | 西班牙语 `es` + 巴西葡语 `pt-BR` | `es-ES`、`es-MX`、拉美票据；PIX、CNPJ、NF-e、BRL | Planned |
| `v1.9.0` | ASC `1.8.0` | Understand & Save | 法语 `fr` + 德语 `de` | 法国 / 加拿大；德国 / 奥地利 / 瑞士；TVA、MwSt、EUR、CAD、CHF | Planned |
| `v2.0` | 待确定 | Archive & Remember | 印尼语 `id` + 越南语 `vi` | QRIS、GoPay、OVO、DANA、MoMo、ZaloPay、IDR、VND | Planned |
| `v2.1` 候选 | 待确定 | Private Intelligence | 印地语 `hi` + 印度英语 `en-IN` profile | UPI、GSTIN、CGST、SGST、IGST、印英混排 | Candidate |
| 后续独立版本 | 待确定 | RTL / Gulf readiness | 阿拉伯语 `ar` + RTL 基础设施 | AED、SAR、VAT、阿英混排票据与酒店水单 | Later |

泰语 `th`、马来语 `ms`、意大利语 `it`、荷兰语 `nl` 等保留在候选池；不得在缺少样本和维护人力时提前绑定公开版本。
...
1. **Version kickoff**：冻结 locale、地区 profile、术语表负责人、样本来源和明确非目标。
...
语言组未完成时可以继续修复版本内其它问题，但不能用“先发布 UI 翻译、以后补识别”绕过门禁。若发布必须延期，应缩减版本内 P1 产品功能，而不是降低语言质量定义。
...
| 文档 | 负责 | 不负责 |
|---|---|---|
| 本文件 | 跨版本语言顺序、主语言策略、通用门禁 | 单一版本完成度、具体测试证据 |
...
- 新版本进入规划时必须在本文件分配语言组；没有语言组的公开功能版本不能进入 Execution Ready。
