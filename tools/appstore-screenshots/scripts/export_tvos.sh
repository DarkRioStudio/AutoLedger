...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --locale)
      ...
      ;;
    *)
      echo "Unknown export_tvos.sh argument: $1" >&2
      exit 2
      ;;
  esac
done
...
print(
    ...
    cfg.get("capture", {}).get("derivedDataPath", "tools/appstore-screenshots/.derivedData") + "-tvos",
...
for line in text.splitlines():
    ...
    if m and m.group(1).startswith("Apple TV"):
        devices.append((m.group(1), m.group(2)))
for candidate in cfg["app"]["tvos"].get("deviceCandidates", []):
...
if devices:
    ...
    raise SystemExit(0)
print("No available Apple TV simulator found.", file=sys.stderr)
raise SystemExit(1)
PY
)
...
if [[ -z "$APP_PATH" || ! -d "$APP_PATH" ]]; then
  echo "Could not find built tvOS app named $PRODUCT_NAME.app under $DERIVED_PATH/Build/Products" >&2
  exit 1
fi
...
capture_tvos_screenshot() {
  ...
  for attempt in $(seq 1 "$attempts"); do
    ...
    echo "warning: tvos/$locale/$shot_id captured incomplete frame; retry $attempt/$attempts" >&2
    sleep 1
  done

  mv "$tmp_path" "$out_path"
  echo "warning: tvos/$locale/$shot_id still looks incomplete after retries" >&2
}

echo "==> Capturing tvOS raw screenshots"
while IFS=$'\t' read -r LOCALE APPLE_LANG APPLE_LOCALE SHOT_ID SCENE; do
  OUT_DIR="$ROOT/tools/appstore-screenshots/output/raw/tvos/$LOCALE"
  mkdir -p "$OUT_DIR"
  OUT_PATH="$OUT_DIR/$SHOT_ID.png"
  echo "capture tvos/$LOCALE/$SHOT_ID ($SCENE)"
  capture_tvos_screenshot "$LOCALE" "$APPLE_LANG" "$APPLE_LOCALE" "$SHOT_ID" "$SCENE" "$OUT_PATH"
done < <(
...
for locale, locale_cfg in cfg["locales"].items():
    if filters and locale not in filters:
        continue
    for shot in cfg["tvosShots"]:
        print(locale, locale_cfg["appleLanguages"], locale_cfg["appleLocale"], shot["id"], shot["scene"], sep="\t")
PY
)

python3 "$SCRIPT_DIR/render_marketing.py" --platform tvos ${LOCALE_FILTERS[@]+"${LOCALE_FILTERS[@]}"}
